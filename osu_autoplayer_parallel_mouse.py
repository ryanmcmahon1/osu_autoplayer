...
import pyautogui
from multiprocessing import Process, Value, Array, Manager, cpu_count
...
active_circles = Manager().list()
active_circles_valid_idx = Value('i', -1, lock=False)
...
class MouseClicker:
    ...
    # based on current mouse location and location of circles,
    ...
    def update_mouse(self):
        ...
        global active_circles, active_circles_valid_idx
        circle_dists = []
        for i, known_circle in enumerate(active_circles):
            if i > active_circles_valid_idx.value:
                break
            ...
            while circle_dists != []:
                ...
                if circle_dists[min_idx] != 1000:
                    ...
                    if circle_dists[min_idx] < THRESHOLD_DIFF:
                        ...
                        pyautogui.leftClick()
                        active_circles[min_idx].clicked = True
                        circle_dists[min_idx] = 1000
                    else:
                        break
                else:
                    break
...
class OsuAutoplayer:
    ...
    def GPIO_callback_27(self, ch):
        ...
        if self.run_autoplayer:
            ...
            active_circles_valid_idx.value = -1
            self.approach_rate = -1
    ...
    def update(self):
        ...
        while not system_exit.value:
            while self.run_autoplayer:
                ...
                small_circles = None
                frame_update = False
                prev_image_tag = self.image_tag_r
                ...
                if loop_time > .2:
                    print(f"Long loop time: {loop_time}; iter {iter_ct}; new_image: {prev_image_tag != tag_read}")
                    print(f"image: {post_image_time - time_stamp}, circle: {circle_update - post_image_time}")
    ...
    def manual_update_approach_radius(self, index, ):
        ...
        prev_radius = active_circles[index].last_outer_radius
        if prev_radius < 0 or prev_radius < active_circles[index].outer_radius:
            return
        decay = self.approach_rate * (time.time() - self.update_timestamp)
        active_circles[index].manual_outer_radius_update(decay)

    def update_circles(self, circles, small_circles, frame_update, delay):
        global active_circles, active_circles_valid_idx
        updated_circles = [False for _ in range(active_circles_valid_idx.value + 1)] # array to track which circles have to be manually updated
        if small_circles is not None:
            for small_circle in small_circles[0,:]:
                ...
                created_circles.append(new_circle)
        ...
        for i, entry in enumerate(updated_circles):
            if not entry:
                if frame_update:
                    active_circles[i].frame_time_accm += self.frame_time
                if self.approach_rate > 0:
                    self.manual_update_approach_radius(i)


        for i, entry in enumerate(active_circles):
            if time.time() - entry.time_stamp > TIME_LIMIT and entry.last_outer_radius < 0:
                active_circles.remove(entry)
                active_circles_valid_idx.value -= 1
            elif entry.clicked == True:
                active_circles.remove(entry)
                active_circles_valid_idx.value -= 1
...
def mouseclick_parallel_process():
    mouseclick_processor = MouseClicker()
...
mouseclick_process = Process(target=mouseclick_parallel_process)
